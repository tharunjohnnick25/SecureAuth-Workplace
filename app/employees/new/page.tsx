        ...
        <main className="pt-20 p-4 sm:p-6 lg:p-8">
          <div className="mb-6">
            <Link href="/employees" className="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-white mb-4 transition-colors">
              <ArrowLeft className="w-4 h-4" /> {'Back to directory'}</Link>
                ...
                <Card className="border-white/10 bg-black/40 backdrop-blur-xl">
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">{'Employee ID'}<span className="text-red-400">*</span></label>
                        <input type="text" value={form.employee_id || ''} onChange={(e) => updateField('employee_id', e.target.value)}
                          className={`w-full bg-white/5 border rounded-xl px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 transition-colors ${errors.employee_id ? 'border-red-500' : 'border-white/10'}`} placeholder="EMP-12345" />
