...
import { Shield, Mail, Lock, Eye, EyeOff, MapPin, Camera, CheckCircle2, XCircle, Loader2, Building2, IdCard } from 'lucide-react';
  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      ...
      const res = await fetch('/api/auth/login', {
        ...
        body: JSON.stringify({
          employee_id: employeeId || data.email, // employee ID issued by admin (fallback to email)
          email: data.email,
