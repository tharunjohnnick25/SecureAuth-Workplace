...
ALTER TABLE public.quorum_requests ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view own quorum requests" ON public.quorum_requests FOR SELECT USING (auth.uid() = requester_id);
CREATE POLICY "Users can insert own quorum requests" ON public.quorum_requests FOR INSERT WITH CHECK (auth.uid() = requester_id);
CREATE POLICY "Admins can view and manage all quorum requests" ON public.quorum_requests FOR ALL USING (true);
...
SELECT public.apply_company_rls('attendance_records');
